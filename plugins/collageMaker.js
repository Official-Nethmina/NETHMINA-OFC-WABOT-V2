...
cmd({
  ...
  filename: __filename,
}, async (nethmina, mek, m, { from, reply, quoted }) => {
  try {
    const imgs = [];
    ...
    if (m.message?.extendedTextMessage?.contextInfo?.quotedMessage?.albumMessage) {
      ...
      for (const item of album) {
        if (item.imageMessage) {
          const buff = await nethmina.downloadMediaMessage(item);
          imgs.push(buff);
        }
      }
    }
    ...
    await nethmina.sendMessage(
      from,
      { image: fs.readFileSync(outFile), caption: "🖼️ Collage created by *NETHMINA-OFC*" },
      { quoted: mek }
    );
