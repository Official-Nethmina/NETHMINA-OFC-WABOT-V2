...
module.exports = {
    ...
    // --- [REPORT TARGET පරාමිතිය මෙතනට එකතු කළා] ---
    onDelete: async (conn, update, reportTarget) => {
        try {
            ...
            // --- [ඔයාගේ INDEX.JS එකේ LOGIC එකට ගැලපෙන පරිදි TARGET එක තෝරා ගැනීම] ---
            const target = reportTarget || from;
            ...
            if (mediaPath && fs.existsSync(mediaPath)) {
                // --- [TARGET එකට අනුව මැසේජ් යැවීම] ---
                if (mediaPath.endsWith('.jpg')) await conn.sendMessage(target, { image: { url: mediaPath }, caption, mentions: [sender] });
                else if (mediaPath.endsWith('.mp4')) await conn.sendMessage(target, { video: { url: mediaPath }, caption, mentions: [sender] });
                else if (mediaPath.endsWith('.webp')) {
                    await conn.sendMessage(target, { sticker: { url: mediaPath } });
                ...
                await conn.sendMessage(target, { text: `${caption}\n\n📝 *Message:* ${text}`, mentions: [sender] });
            }
